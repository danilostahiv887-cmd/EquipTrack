...
export type RoomActionState = { formError?: string; fieldErrors?: Record<string, string[]>; values?: Record<string, string> };
...
export async function updateRoomAction(_: RoomActionState, formData: FormData): Promise<RoomActionState> {
  const user = await getCurrentUser();
  if (!user) return { formError: "Сеанс завершено. Увійдіть повторно." };
  try { assertPermission(user, "room:manage"); } catch (error) { return { formError: error instanceof Error ? error.message : "Доступ заборонено." }; }
  ...
  await withDatabase((db) => db.query("UPDATE $id MERGE $value;", { id, value: { ...parsed.data, updatedAt: new Date().toISOString() } }));
  revalidatePath("/rooms");
  revalidatePath(`/rooms/${encodeURIComponent(normalizeRecordIdString(roomId))}`);
  return {};
}
...
export async function deleteRoomAction(formData: FormData) {
  ...
  await withDatabase((db) => db.query("BEGIN TRANSACTION; DELETE $id; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;", {
    id,
    log: { actorId: user.id, action: "room.deleted", entityType: "room", entityId: textId, createdAt: new Date().toISOString() },
  }));
  revalidatePath("/rooms");
