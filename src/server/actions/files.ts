...
async function refreshOwnerPhoto(
  entityType: string,
  entityId: string,
  ...
  await withDatabase(async (db) => {
    const [owner] = await queryRows<PhotoOwner>(
      db,
      `SELECT id, ${photoField} FROM ${ownerTable} WHERE id = $id LIMIT 1;`,
      { id: ownerRecord },
    );
    if (!owner) return;
    const currentPhoto = normalizeRecordIdString(
      String(
        entityType === "category"
          ? (owner.imageFileId ?? "")
          : (owner.photoFileId ?? ""),
    ...
    const [nextPhoto] = await queryRows<{ id: unknown; createdAt?: string }>(
      db,
      "SELECT id, createdAt FROM file WHERE entityId = $entityId AND id != $fileId AND kind = 'photo' ORDER BY createdAt DESC LIMIT 1;",
      { entityId, fileId: toRecordId(removedFileId) },
    );
    await db.query(`UPDATE $owner SET ${photoField} = $nextPhoto;`, {
      owner: ownerRecord,
      nextPhoto: nextPhoto ? recordId(nextPhoto.id) : null,
    });
  });
}
...
export async function deleteFileAction(formData: FormData) {
  ...
  if (!fileId || fileId === "undefined")
    throw new Error("Не вказано файл для видалення.");

  const fileRecord = toRecordId(fileId);
  const [file] = await withDatabase((db) =>
    queryRows<StoredFile>(
      db,
      "SELECT id, entityType, entityId FROM file WHERE id = $id LIMIT 1;",
      { id: fileRecord },
    ),
  );
  ...
  if (entityType && entityId)
    await refreshOwnerPhoto(entityType, entityId, fileId);

  revalidatePath(returnPath);
  revalidatePath("/documents");
  if (entityType === "equipment" && entityId)
    revalidatePath(`/equipment/${encodeURIComponent(entityId)}`);
  if (entityType === "room" && entityId)
    revalidatePath(`/rooms/${encodeURIComponent(entityId)}`);
}
