...
export type WorkflowActionState = { formError?: string; fieldErrors?: Record<string, string[]>; success?: string; values?: Record<string, string> };
...
const initialError = (error: unknown, formData?: FormData) => ({ formError: error instanceof Error ? error.message : "Операцію не виконано.", ...(formData ? { values: formValues(formData) } : {}) });
...
const equipmentId = requiredText("Оберіть екземпляр обладнання.").regex(/^equipment_instance:[A-Za-z0-9_-]+$/, "Некоректний екземпляр обладнання.");
const roomId = requiredText("Оберіть приміщення.").regex(/^room:[A-Za-z0-9-]+$/, "Некоректне приміщення.");
const transferSchema = z.object({ equipmentId, fromRoomId: roomId, toRoomId: roomId, reason: requiredText("Опишіть причину передачі.").min(4, "Причина має містити щонайменше 4 символи.") });
const requiredNumber = (message: string) => z.preprocess((value) => String(value ?? "").trim() === "" ? undefined : value, z.coerce.number({ required_error: message, invalid_type_error: "Кількість має бути числом." }));
const movementSchema = z.object({
  equipmentId,
  toRoomId: roomId,
  movementType: z.enum(["transferred", "returned_to_storage", "sent_to_repair", "returned_from_repair", "corrected"], {
    required_error: "Оберіть тип руху.",
    invalid_type_error: "Оберіть тип руху.",
  }),
  reason: requiredText("Опишіть причину руху.").min(4, "Причина має містити щонайменше 4 символи."),
});
const repairSchema = z.object({ equipmentId, roomId, issueDescription: requiredText("Опишіть несправність.").min(6, "Опис несправності має містити щонайменше 6 символів."), severity: z.enum(["low", "medium", "high"], { required_error: "Оберіть серйозність несправності.", invalid_type_error: "Оберіть серйозність несправності." }) });
const recordRef = (message: string) => z.string({ required_error: message, invalid_type_error: message }).trim().min(1, message).transform((value) => toRecordId(value));
const auditSchema = z.object({
  title: requiredText("Вкажіть назву аудиту.").min(4, "Назва аудиту має містити щонайменше 4 символи."),
  roomId,
  plannedDate: requiredText("Вкажіть заплановану дату."),
  auditScope: requiredText("Опишіть перелік або обсяг перевірки.").min(6, "Обсяг перевірки має містити щонайменше 6 символів."),
  expectedItemCount: requiredNumber("Вкажіть очікувану кількість позицій.").pipe(z.number().int("Кількість має бути цілим числом.").min(0, "Кількість не може бути від’ємною.").max(999, "Кількість завелика для одного аудиту.")),
  auditNote: z.string().trim().max(600, "Примітка має містити не більше 600 символів.").optional(),
});
...
export async function createTransferRequestAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  ...
  try { assertPermission(user, "request:create"); } catch (error) { return initialError(error, formData); }
  const parsed = transferSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return fieldErrors(parsed.error, formData);
...
export async function createMovementAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  const user = await getCurrentUser(); if (!user) return { formError: "Сеанс завершено." };
  try { assertPermission(user, "movement:manage"); } catch (error) { return initialError(error, formData); }
  const parsed = movementSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return fieldErrors(parsed.error, formData);
  let modelId = "";
  try {
    await withDatabase(async (db) => {
      const [equipment] = await queryRows<{ id: unknown; equipmentId?: string; currentRoomId?: string; currentResponsibleId?: string; status?: string }>(
        db,
      ...
      if (equipment.status === "written_off") throw new Error("Списане обладнання не можна переміщувати.");
      modelId = normalizeRecordIdString(equipment.equipmentId ?? "");
      const fromRoomId = normalizeRecordIdString(equipment.currentRoomId ?? "");
      if (fromRoomId === parsed.data.toRoomId) throw new Error("Оберіть інше приміщення: обладнання вже знаходиться там.");
      ...
      await db.query(
        ...
        COMMIT TRANSACTION;`,
        {
          ...
          log: { actorId: user.id, action: "movement.created", entityType: "equipment_instance", entityId: parsed.data.equipmentId, createdAt: timestamp },
        },
      );
    });
  } catch (error) {
...
export async function createRepairAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  const user = await getCurrentUser(); if (!user) return { formError: "Сеанс завершено." };
  try { assertPermission(user, "repair:report"); } catch (error) { return initialError(error, formData); }
  const parsed = repairSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return fieldErrors(parsed.error, formData);
  const timestamp = new Date().toISOString();
...
export async function transitionRepairAction(formData: FormData) {
  ...
  await withDatabase(async (db) => {
    const [repair] = await queryRows<{ id: unknown; equipmentId: string; roomId?: string; status: string; issueDescription?: string; severity?: string }>(db, "SELECT * FROM repair WHERE id = $id LIMIT 1;", { id: parsed.repairId });
    if (!repair) throw new Error("Заявку на ремонт не знайдено.");
    const allowed: Record<string, string[]> = {
      reported: ["under_review", "cancelled"],
      under_review: ["sent_to_repair", "repaired", "not_repairable", "cancelled"],
      sent_to_repair: ["repaired", "not_repairable", "cancelled"],
      repaired: [],
      not_repairable: [],
      cancelled: [],
    };
    if (!allowed[repair.status]?.includes(targetStatus)) throw new Error("Такий перехід стану ремонту недоступний.");
    const timestamp = new Date().toISOString();
    const movementId = ["sent_to_repair", "repaired"].includes(targetStatus) ? generated("movement") : "";
    const equipmentPatch = targetStatus === "sent_to_repair"
      ? { status: "in_repair", condition: "needs_repair", updatedAt: timestamp }
      : targetStatus === "repaired"
        ? { status: "active", condition: "good", updatedAt: timestamp }
        : targetStatus === "not_repairable"
          ? { status: "in_storage", condition: "unusable", updatedAt: timestamp }
          : undefined;
    const movementType = targetStatus === "sent_to_repair" ? "sent_to_repair" : targetStatus === "repaired" ? "returned_from_repair" : "";
    const statements = [
      "BEGIN TRANSACTION;",
    ...
    await db.query(statements.join(" "), {
      ...
      actor: user.id,
      time: timestamp,
      movement: movementType ? { equipmentId: repair.equipmentId, movementType, fromRoomId: repair.roomId, toRoomId: repair.roomId, performedBy: user.id, movementDate: timestamp, reason: repair.issueDescription ?? "Операція ремонту", createdAt: timestamp } : null,
      log: { actorId: user.id, action: "repair.updated", entityType: "repair", entityId: normalizeRecordIdString(parsed.repairId), createdAt: timestamp },
    });
  });
  revalidatePath("/repairs"); revalidatePath("/equipment"); revalidatePath("/movements"); revalidatePath("/dashboard");
}

export async function markNotificationReadAction(formData: FormData) {
  const user = await getCurrentUser(); if (!user) return;
  const notificationId = recordRef("Некоректне сповіщення.").parse(formData.get("notificationId"));
  await withDatabase((db) => db.query("UPDATE $id MERGE { isRead: true, readAt: $time };", { id: notificationId, time: new Date().toISOString() }));
...
export async function decideTransferRequestAction(formData: FormData) {
  ...
  await withDatabase(async (db) => {
    ...
    const decisionPatch = decision === "approved" ? { approvedAt: time } : { rejectedAt: time };
    await db.query("UPDATE $id MERGE $value;", { id, value: { status: decision, approvedBy: user.id, updatedAt: time, ...decisionPatch } });
...
export async function createAuditAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  ...
  const parsed = auditSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return fieldErrors(parsed.error, formData);
  const timestamp = new Date().toISOString(); const auditId = generated("audit");
  await withDatabase(async (db) => {
    const assets = await queryRows<{ id: unknown; currentRoomId: string; condition: string; serialNumber?: string; inventoryNumber?: string }>(db, "SELECT id, currentRoomId, condition, serialNumber, inventoryNumber FROM equipment_instance WHERE currentRoomId = $roomId;", { roomId: parsed.data.roomId });
    await db.query(`BEGIN TRANSACTION; CREATE ${auditId} CONTENT $audit; ${assets.map((_, index) => `CREATE audit_item:item${index}${auditId.replace("audit:", "")} CONTENT $item${index};`).join(" ")} COMMIT TRANSACTION;`, { audit: { ...parsed.data, auditNote: parsed.data.auditNote || undefined, actualItemCount: 0, expectedRegisteredCount: assets.length, status: "planned", createdBy: user.id, createdAt: timestamp, updatedAt: timestamp }, ...Object.fromEntries(assets.map((asset, index) => [`item${index}`, { auditId, equipmentId: normalizeRecordIdString(asset.id), expectedRoomId: asset.currentRoomId, expectedSerialNumber: asset.serialNumber, expectedInventoryNumber: asset.inventoryNumber, expectedCondition: asset.condition, resultStatus: "pending", createdAt: timestamp }])) });
  });
  revalidatePath("/audits"); return { success: "Аудит створено." };
}

export async function updateAuditAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  const user = await getCurrentUser(); if (!user) return { formError: "Сеанс завершено." };
  try { assertPermission(user, "audit:manage"); } catch (error) { return initialError(error, formData); }
  const parsed = auditUpdateSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return fieldErrors(parsed.error, formData);
  const timestamp = new Date().toISOString();
  try {
    await withDatabase(async (db) => {
      const [audit] = await queryRows<{ status: string }>(db, "SELECT status FROM audit WHERE id = $id LIMIT 1;", { id: parsed.data.auditId });
      if (!audit) throw new Error("Аудит не знайдено.");
      if (audit.status === "completed") throw new Error("Завершений аудит не можна редагувати.");
...
export async function startAuditAction(formData: FormData) {
  ...
  const timestamp = new Date().toISOString();
  await withDatabase(async (db) => {
    const [audit] = await queryRows<{ status: string }>(db, "SELECT status FROM audit WHERE id = $id LIMIT 1;", { id: parsed.auditId });
    if (!audit) throw new Error("Аудит не знайдено.");
    if (audit.status !== "planned") throw new Error("Почати можна лише запланований аудит.");
    await db.query("BEGIN TRANSACTION; UPDATE $id MERGE { status: 'in_progress', startedAt: $time, updatedAt: $time, checkedBy: $actor }; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;", {
      id: parsed.auditId,
      time: timestamp,
      actor: user.id,
      log: { actorId: user.id, action: "audit.started", entityType: "audit", entityId: normalizeRecordIdString(parsed.auditId), createdAt: timestamp },
    });
  });
  revalidatePath("/audits");
  revalidatePath("/dashboard");
...
export async function cancelAuditAction(formData: FormData) {
  ...
  await withDatabase(async (db) => {
    const [audit] = await queryRows<{ status: string }>(db, "SELECT status FROM audit WHERE id = $id LIMIT 1;", { id: parsed.auditId });
    if (!audit) throw new Error("Аудит не знайдено.");
    if (!["planned", "in_progress"].includes(audit.status)) throw new Error("Скасувати можна лише запланований або активний аудит.");
    await db.query("BEGIN TRANSACTION; UPDATE $id MERGE { status: 'cancelled', cancelledAt: $time, updatedAt: $time, checkedBy: $actor }; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;", {
      id: parsed.auditId,
      time: timestamp,
      actor: user.id,
      log: { actorId: user.id, action: "audit.cancelled", entityType: "audit", entityId: normalizeRecordIdString(parsed.auditId), createdAt: timestamp },
    });
  });
  revalidatePath("/audits");
  revalidatePath("/dashboard");
...
export async function deleteAuditAction(formData: FormData) {
  ...
  await withDatabase((db) => db.query("BEGIN TRANSACTION; DELETE $id; DELETE audit_item WHERE auditId = $textId; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;", {
    id: parsed.auditId,
    textId,
    log: { actorId: user.id, action: "audit.deleted", entityType: "audit", entityId: textId, createdAt: new Date().toISOString() },
  }));
  revalidatePath("/audits");
  revalidatePath("/dashboard");
}
...
export async function scanAuditItemAction(_: WorkflowActionState, formData: FormData): Promise<WorkflowActionState> {
  ...
  try {
    await withDatabase(async (db) => {
      ...
      const [audit] = await queryRows<{ id: unknown; roomId: string; status: string }>(db, "SELECT id, roomId, status FROM audit WHERE id = $id LIMIT 1;", { id: parsed.data.auditId });
      if (!audit) throw new Error("Аудит не знайдено.");
      ...
      if (!equipmentRows.length) {
        ...
        await db.query(`BEGIN TRANSACTION; CREATE ${itemId} CONTENT $item; UPDATE $audit MERGE { updatedAt: $time }; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;`, {
          ...
          item: {
            ...
            note: parsed.data.note || "Номер не знайдено в реєстрі обладнання.",
            checkedBy: user.id,
            checkedAt: timestamp,
            createdAt: timestamp,
          },
...
export async function finishAuditAction(formData: FormData) {
  ...
  await withDatabase(async (db) => {
    ...
    const [audit] = await queryRows<{ status: string }>(db, "SELECT status FROM audit WHERE id = $id LIMIT 1;", { id: parsed.auditId });
    if (!audit) throw new Error("Аудит не знайдено.");
    if (audit.status !== "in_progress") throw new Error("Завершити можна лише аудит у роботі.");
    await db.query("UPDATE audit_item SET resultStatus = 'missing', checkedBy = $actor, checkedAt = $time, updatedAt = $time WHERE auditId = $auditId AND (resultStatus = NONE OR resultStatus = 'pending');", { auditId: auditText, actor: user.id, time: timestamp });
    const items = await queryRows<{ resultStatus?: string; actualCondition?: string }>(db, "SELECT resultStatus, actualCondition FROM audit_item WHERE auditId = $auditId;", { auditId: auditText });
    const count = (status: string) => items.filter((item) => item.resultStatus === status).length;
    ...
    await db.query("BEGIN TRANSACTION; UPDATE $id MERGE { status: 'completed', completedAt: $time, updatedAt: $time, checkedBy: $actor, actualItemCount: $actualItemCount, auditResult: $auditResult }; CREATE audit_log CONTENT $log; COMMIT TRANSACTION;", {
      id: parsed.auditId,
      time: timestamp,
      actor: user.id,
      actualItemCount,
      auditResult,
      log: { actorId: user.id, action: "audit.completed", entityType: "audit", entityId: auditText, createdAt: timestamp },
    });
  });
  revalidatePath("/audits");
  revalidatePath("/dashboard");
