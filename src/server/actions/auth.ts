...
const loginSchema = z.object({
  email: requiredText("Введіть електронну адресу.").email("Введіть коректну електронну адресу."),
...
export async function loginAction(_: LoginState, formData: FormData): Promise<LoginState> {
  ...
  try {
    const authenticated = await withDatabase(async (db) => {
      const users = await queryRows<Record<string, unknown>>(db, "SELECT id, email, passwordHash, status FROM user WHERE email = $email LIMIT 1;", { email: parsed.data.email.toLowerCase() });
      const user = users[0];
      if (!user || user.status !== "active" || !(await verifyPassword(parsed.data.password, String(user.passwordHash)))) return false;
      await createSession({ userId: user.id, email: String(user.email) }, db);
      return true;
    });
    if (!authenticated) return { formError: "Неправильна електронна адреса або пароль." };
  } catch {
