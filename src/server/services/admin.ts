...
export async function getUsers(
  ...
  return withDatabase(async (db) => {
    ...
    const rows = await queryRows<Record<string, unknown>>(
      db,
      "SELECT id, fullName, email, role, status, position, createdAt FROM user ORDER BY fullName;",
    );
    const needle = filters?.q?.trim().toLowerCase();
    const filtered = rows.filter((item) => {
      if (
        needle &&
        ![item.fullName, item.email, item.position, item.role, item.status]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
          .includes(needle)
...
export async function getDocuments(
  page: number,
  filters?: string | { q?: string; type?: string; kind?: string },
): Promise<Page<FileAttachment>> {
  return withDatabase(async (db) => {
    const pageSize = 12;
    const safePage = Math.max(1, page);
    const options =
      typeof filters === "string" ? { q: filters } : (filters ?? {});
    const rows = await queryRows<FileAttachment>(
      db,
      "SELECT id, name, mimeType, size, kind, entityType, entityId, createdAt FROM file ORDER BY createdAt DESC;",
    );
