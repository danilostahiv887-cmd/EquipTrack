...
export function buildLookupSet({
  ...
  for (const model of equipmentModels)
    equipment.set(idKey(model.id), { id: model.id, name: model.name });
...
function auditSummary(row: DisplayRow, lookup: LookupSet) {
  ...
  const expectedRegisteredCount = items.filter(
    (item) => item.equipmentId && item.resultStatus !== "unknown",
  ).length;
  const pending = items.filter(
    (item) => !item.resultStatus || item.resultStatus === "pending",
  ).length;
  const found = items.filter((item) => item.resultStatus === "found").length;
  const misplaced = items.filter(
    (item) => item.resultStatus === "misplaced",
  ).length;
  const unknown = items.filter(
    (item) => item.resultStatus === "unknown",
  ).length;
  ...
  const missing = items.filter(
    (item) => item.resultStatus === "missing",
  ).length;
  const actualCount = found + misplaced + unknown + damaged;
  const problemItems = items.filter(
    (item) =>
      ["misplaced", "missing", "unknown", "damaged"].includes(
        String(item.resultStatus ?? ""),
      ) ||
      item.actualCondition === "damaged" ||
      item.actualCondition === "unusable",
  );
  const preview = items
    ...
    .map((item) => {
      ...
      const serial = item.expectedSerialNumber
        ...
        : item.scannedCode
          ? `введено ${item.scannedCode}`
          : "";
      const status = label(
        item.resultStatus ?? "pending",
        String(item.resultStatus ?? "pending"),
      );
      const location =
        item.resultStatus === "misplaced"
  ...
  const problems = problemItems
    ...
    .map((item) => {
      ...
      const condition = item.actualCondition
        ? `стан: ${label(item.actualCondition, String(item.actualCondition))}`
        : "";
      const location =
        item.resultStatus === "misplaced"
          ? `має бути: ${describeRoom(item.expectedRoomId, lookup)}; знайдено: ${describeRoom(item.actualRoomId, lookup)}`
          : "";
      return [equipment, status, condition, location, item.note]
        .filter(Boolean)
        .join(" · ");
    })
    .join("; ");
  return {
    ...
    problems: problems
      ? `Проблемні позиції (${problemItems.length}): ${problems}${problemItems.length > 10 ? "…" : ""}`
      : "Розбіжностей не зафіксовано.",
  };
}

function auditCountDelta(expected: unknown, actual: number) {
  if (expected == null || expected === "")
    return "Очікувану кількість не вказано.";
...
function decorateRow(
  ...
  const set = (key: string, value: string) => {
    if (row[key] != null && row[key] !== "") display[key] = value;
  };
  ...
  if (row.movementType)
    display.movementType = label(row.movementType, String(row.movementType));
  ...
  if (kind === "audits") {
    ...
    row.itemCountDelta = auditCountDelta(
      row.expectedItemCount,
      Number(row.actualItemCount ?? summary.actualCount),
    );
  ...
  const movement = row.movementType
    ? label(row.movementType, String(row.movementType))
    : "";

  if (kind === "movements" || row.movementType) {
    row.__title = equipment
      ? `${movement || "Рух"} · ${equipment}`
      : movement || "Рух обладнання";
    row.__subtitle = [
      from && `з ${from}`,
      to && `до ${to}`,
    ...
    row.__title = equipment
      ? `Заявка: ${equipment}`
      : String(row.reason ?? "Заявка на передачу");
    row.__subtitle = [from && `з ${from}`, to && `до ${to}`, status]
      .filter(Boolean)
      .join(" · ");
  } else if (kind === "repairs") {
    row.__title = equipment
      ? `Ремонт: ${equipment}`
      : String(row.issueDescription ?? "Повідомлення про ремонт");
    row.__subtitle = [
      row.issueDescription && String(row.issueDescription),
      row.severity &&
        `Серйозність: ${label(row.severity, String(row.severity))}`,
      status,
    ]
      .filter(Boolean)
      .join(" · ");
    ...
    row.__subtitle = [room, status, row.auditResult && String(row.auditResult)]
      .filter(Boolean)
      .join(" · ");
  } else if (kind === "writeoffs") {
    const writeoffProgress: Record<string, string> = {
      proposed:
        "Запропоновано → очікує рішення адміністратора → фактичне списання",
      approved:
        "Запропоновано → погоджено → очікує підтвердження фактичного списання",
    ...
    row.writeoffProgress =
      writeoffProgress[String(row.status ?? "")] ??
      "Маршрут списання не визначено.";
    row.__title = equipment
      ? `Списання: ${equipment}`
      : String(row.reason ?? "Пропозиція списання");
    row.__subtitle = [row.reason && String(row.reason), status]
      .filter(Boolean)
      .join(" · ");
    ...
    row.__subtitle = [
      ...
      display.userId && `для ${display.userId}`,
    ]
      .filter(Boolean)
      .join(" · ");
  } else if (kind === "auditLog") {
    row.__title =
      [row.action && label(row.action, String(row.action)), display.entityId]
        .filter(Boolean)
        .join(" · ") || "Запис журналу";
    row.__subtitle = [
      display.actorId && `виконав ${display.actorId}`,
  ...
  row.__search = Object.values({ ...row, ...display })
...
export async function enrichWorkflowRows(
  db: Surreal,
  rows: DisplayRow[],
  kind?: string,
) {
...
export function enrichWorkflowRowsWithLookup(
  rows: DisplayRow[],
  lookup: LookupSet,
  kind?: string,
) {
...
export async function enrichFiles(db: Surreal, files: FileAttachment[]) {
  ...
  return files.map((file) => {
    ...
    const normalizedId = entityId
      ? normalizeRecordIdString(String(entityId))
      : "";
    const targetHref =
