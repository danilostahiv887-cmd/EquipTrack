...
export async function createSession(
  ...
  const createRecord = (db: Surreal) =>
    db.query("CREATE session CONTENT $value;", { value });
...
export async function clearSession() {
  ...
  if (token) {
    await withDatabase(async (db) => {
      await db.query("DELETE session WHERE tokenHash = $hash;", {
        hash: digest(token),
      });
    }).catch(() => undefined);
  }
  ...
  try {
    return await withDatabase(async (db) => {
      const sessions = await queryRows<{
        ...
        db,
        "SELECT userId, userEmail FROM session WHERE tokenHash = $hash AND expiresAt > $now LIMIT 1;",
        { hash: digest(token), now: new Date().toISOString() },
      );
      ...
      const users = session.userEmail
        ? await queryRows<Record<string, unknown>>(
            db,
            "SELECT id, fullName, email, role, status FROM user WHERE email = $email LIMIT 1;",
            { email: session.userEmail },
          )
        : await queryRows<Record<string, unknown>>(
            db,
            "SELECT id, fullName, email, role, status FROM user WHERE id = $id LIMIT 1;",
            { id: toRecordId(session.userId) },
          );
      const user = users[0];
      ...
      return {
        ...
        role: user.role as WorkspaceUser["role"],
        status: "active",
      };
    });
  } catch {
