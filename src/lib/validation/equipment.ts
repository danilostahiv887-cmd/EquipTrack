...
export const equipmentSchema = z.object({
  ...
  condition: z
    .enum(
      ["new", "good", "satisfactory", "needs_repair", "damaged", "unusable"],
      {
        required_error: "Оберіть типовий стан обладнання.",
        invalid_type_error: "Оберіть типовий стан обладнання.",
      },
    )
    .default("good"),
});
...
export const equipmentInstanceSchema = z.object({
  equipmentId: text("Оберіть картку обладнання.").regex(
    /^equipment:[A-Za-z0-9_-]+$/,
    "Некоректна картка обладнання.",
  ),
  inventoryNumber: text("Вкажіть інвентарний номер екземпляра.").min(
    2,
    "Інвентарний номер має містити щонайменше 2 символи.",
  ),
  serialNumber: text("Вкажіть серійний номер екземпляра.")
    .min(2, "Серійний номер має містити щонайменше 2 символи.")
    .max(120),
  ...
  status: z
    .enum(
      ...
      {
        required_error: "Оберіть обліковий стан екземпляра.",
        invalid_type_error: "Оберіть обліковий стан екземпляра.",
      },
    )
    .default("active"),
  condition: z.enum(
    ["new", "good", "satisfactory", "needs_repair", "damaged", "unusable"],
    {
      required_error: "Оберіть стан обладнання.",
      invalid_type_error: "Оберіть стан обладнання.",
    },
  ),
...
export const equipmentInstanceUpdateSchema = equipmentInstanceSchema.extend({
  instanceId: text("Не вказано екземпляр обладнання.").regex(
