...
export async function prepareUpload(upload: File): Promise<PreparedFile> {
  if (!upload.type || !allowedTypes.has(upload.type))
    throw new Error("Дозволено завантажувати лише JPEG, PNG, WebP або PDF.");
  if (upload.size <= 0) throw new Error("Вибраний файл порожній.");
  if (upload.size > MAX_FILE_BYTES)
    throw new Error("Розмір файлу перевищує 3 МБ.");
  ...
  const preview = await sharp(original.data)
    .resize({ width: 520, height: 390, fit: "cover" })
    .webp({ quality: 74 })
    .toBuffer();
  return {
    name: upload.name.replace(/\.[^.]+$/, ".webp"),
    mimeType: "image/webp",
