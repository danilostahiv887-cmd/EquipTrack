    ...
    <div className="ledger-table equipment-ledger equipment-model-ledger">
      ...
      {equipment.map((item) => {
        ...
        return (
          <div className="ledger-row ledger-row-actions" key={id}>
            <Link
              className="ledger-primary-link equipment-label"
              href={`/equipment/${encodeURIComponent(id)}`}
            >
              {item.photoFileId ? (
                <img
                  src={`/api/files/${encodeURIComponent(item.photoFileId)}/preview`}
                  alt=""
                />
              ) : (
                <i>Немає фото</i>
            ...
            <span>
              {money.format(Number(item.totalValue || item.price || 0))}
            </span>
                ...
                <>
                  ...
                  <form action={deleteEquipmentAction}>
                    ...
                    <ConfirmSubmit
                      ...
                      description={`Картку ${item.name} буде прибрано з реєстру разом з її екземплярами.`}
      ...
      {equipment.length === 0 && (
        <div className="empty-ledger">
          За цими фільтрами обладнання не знайдено.
        </div>
      )}
    </div>
  );
