...
function instanceSummary(instances: EquipmentInstance[]) {
  ...
  const repair = instances.filter(
    (item) =>
      item.status === "in_repair" ||
      item.condition === "needs_repair" ||
      item.condition === "damaged",
  ).length;
  const storage = instances.filter(
...
export function EquipmentPassport({
  data,
  references,
  canManage,
}: {
  ...
  return (
    <section className="passport equipment-passport">
      <div className="passport-photo">
        {item.photoFileId ? (
          <img
            src={`/api/files/${encodeURIComponent(item.photoFileId)}/full`}
            alt={`Фото: ${item.name}`}
          />
        ) : (
          <p>Фотографія відсутня</p>
        )}
      </div>
      <div className="passport-title">
        ...
        {canManage && (
          <div className="row-actions">
            <Dialog
              label="Редагувати картку"
              title="Редагування картки обладнання"
            >
              <EquipmentForm
                mode="edit"
                equipment={item}
            ...
            <Dialog
              label="Додати екземпляр"
              title="Новий фізичний екземпляр"
              icon={false}
              triggerClassName="inline-dialog-trigger action-stamp"
            >
            ...
            <form action={deleteEquipmentAction}>
              ...
              <ConfirmSubmit
                label="Видалити картку"
                title="Видалити картку обладнання?"
                description={`Картку ${item.name} буде прибрано разом з її екземплярами.`}
      ...
      <div className="technical-strip">
        <span>
          Екземплярів <b>{data.instances.length}</b>
        </span>
        <span>
          Активних <b>{summary.active}</b>
        </span>
        <span>
          Потребують уваги <b>{summary.repair}</b>
        </span>
        <span>
          Балансова вартість{" "}
          <b>{money.format(summary.total || Number(item.price ?? 0))}</b>
        </span>
      </div>
      ...
      <section className="instance-section">
        <div className="section-toolbar">
          <div>
            ...
            <p>
              Кожна фізична одиниця має власний серійний номер, інвентарний
              номер, приміщення та відповідальну особу.
            </p>
          </div>
          <form className="filter-line compact-filter">
            <input
              name="instances"
              defaultValue={data.instanceQuery}
              placeholder="Пошук: серійний, інвентарний, приміщення, відповідальний"
            />
        ...
        <div className="instance-list">
              ...
              <article className="instance-card" key={instanceId}>
                ...
                <div className="row-actions">
                  {instance.currentRoomId && (
                    <Link
                      href={`/rooms/${encodeURIComponent(recordId(instance.currentRoomId))}`}
                    >
                      Приміщення
                    </Link>
                  )}
                  {canManage && (
                    <>
                      <Dialog
                        label="Редагувати"
                        title={`Екземпляр ${instance.inventoryNumber}`}
                        icon={false}
                      ...
                      <form action={deleteEquipmentInstanceAction}>
                        ...
                        <ConfirmSubmit
                          ...
                          description={`Екземпляр ${instance.inventoryNumber} буде прибрано з картки.`}
                          confirmLabel="Так, видалити"
                          tone="danger"
                        />
                      </form>
          ...
          {data.filteredInstances.length === 0 && (
            <p className="empty-ledger">
              Екземпляри за цим пошуком не знайдені.
            </p>
          )}
        </div>
      </section>
      ...
      <section>
        ...
        <FileGallery
          files={data.files}
          canManage={canManage}
          returnPath={`/equipment/${encodeURIComponent(id)}`}
        />
      </section>
          ...
          <div>
            ...
            <span>
              {item.acquisitionDate
                ? date.format(new Date(item.acquisitionDate))
                : "—"}
            </span>
            <StatusBadge status={item.status} />
          </div>
          <div>
            <strong>Типова ціна одиниці</strong>
