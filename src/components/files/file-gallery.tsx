        ...
        return (
          <article className="file-card" key={id}>
            <Dialog
              ...
              title={title}
              icon={false}
              triggerClassName="file-card-preview"
            >
              <div className="file-viewer">
                {isImage(file.mimeType) ? (
                  <img src={fullUrl} alt={title} />
                ) : (
                  <p>Цей файл відкривається в окремій вкладці.</p>
                )}
                <Link
            ...
            <div className="file-card-copy">
              <strong>{title}</strong>
              <span>
                {String(file.mimeType ?? "Файл")} ·{" "}
                {Number(file.size ?? 0).toLocaleString("uk-UA")} байт
              </span>
            </div>
              ...
              {canManage && (
                <form action={deleteFileAction}>
                  ...
                  <ConfirmSubmit
                    ...
                    description={`Файл «${title}» буде прибрано з цієї картки.`}
