...
const groups: Array<{ title: string; modules: Module[] }> = [
  { title: "Робочий простір", modules: [
    { slug: "dashboard", label: "Огляд", code: "ОГ", roles: ["staff", "inventory_manager", "admin"] },
    { slug: "notifications", label: "Сповіщення", code: "СП", roles: ["staff", "inventory_manager", "admin"] },
  ] },
  { title: "Облік", modules: [
    { slug: "rooms", label: "Приміщення", code: "ПР", roles: ["staff", "inventory_manager", "admin"] },
    { slug: "equipment", label: "Обладнання", code: "ОБ", roles: ["staff", "inventory_manager", "admin"] },
    { slug: "movements", label: "Рухи", code: "РУ", roles: ["inventory_manager", "admin"] },
    { slug: "documents", label: "Документи", code: "ДО", roles: ["inventory_manager", "admin"] },
  ] },
  { title: "Контроль", modules: [
    { slug: "requests", label: "Заявки", code: "ЗА", roles: ["staff", "inventory_manager", "admin"] },
    { slug: "repairs", label: "Ремонти", code: "РЕ", roles: ["inventory_manager", "admin"] },
    { slug: "audits", label: "Аудити", code: "АУ", roles: ["inventory_manager", "admin"] },
    { slug: "writeoffs", label: "Списання", code: "СЛ", roles: ["inventory_manager", "admin"] },
  ] },
  { title: "Адміністрування", modules: [
    { slug: "analytics", label: "Аналітика", code: "АН", roles: ["inventory_manager", "admin"] },
...
export function CatalogIndex({ role }: { role: Role }) {
  ...
  useEffect(() => {
    setCollapsed(window.localStorage.getItem("equiptrack.catalog.collapsed") === "1");
  }, []);
  ...
  return <>
    ...
    <nav className={`catalog-index ${opened ? "catalog-index-open" : ""} ${collapsed ? "catalog-collapsed" : ""}`} aria-label="Розділи системи">
      <button className="catalog-dock-toggle" type="button" aria-expanded={!collapsed} aria-label={collapsed ? "Розгорнути картотеку" : "Згорнути картотеку"} title={collapsed ? "Розгорнути картотеку" : "Згорнути картотеку"} onClick={toggleCollapsed}>
        <span className="catalog-arrow-glyph" aria-hidden="true">{collapsed ? "‹‹" : "››"}</span>
