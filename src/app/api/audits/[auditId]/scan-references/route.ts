...
export async function GET(_: Request, { params }: { params: Promise<{ auditId: string }> }) {
  ...
  if (!can(user, "audit:manage")) return NextResponse.json({ error: "Недостатньо прав." }, { status: 403 });

  const { auditId } = await params;
  const data = await getAuditScanReferences(auditId);
  return NextResponse.json({ equipment: data.equipment, auditItems: data.auditItems }, { headers: { "Cache-Control": "private, no-store" } });
}
