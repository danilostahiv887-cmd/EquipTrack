...
function asciiFilename(value: unknown) {
...
function contentDisposition(value: unknown) {
  const filename = String(value || "file");
  return `inline; filename="${asciiFilename(filename)}"; filename*=UTF-8''${encodeURIComponent(filename)}`;
}
...
export async function GET(_: Request, { params }: { params: Promise<{ fileId: string; variant: string }> }) {
  ...
  const file = await withDatabase(async (db) => (await queryRows<Record<string, unknown>>(db, "SELECT * FROM file WHERE id = $id LIMIT 1;", { id: toRecordId(decodeURIComponent(fileId)) }))[0]).catch(() => undefined);
  ...
  return new NextResponse(responseBody, { headers: { "Content-Type": String(file.mimeType || "application/octet-stream"), "Content-Length": String(body.byteLength), "Content-Disposition": variant === "preview" ? "inline" : contentDisposition(file.name), "Cache-Control": variant === "preview" ? "private, max-age=3600" : "private, no-store" } });
}
