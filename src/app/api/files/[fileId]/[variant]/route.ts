...
export async function GET(
  ...
  if (!user || !can(user, "equipment:read"))
    ...
    async (db) =>
      (
        await queryRows<Record<string, unknown>>(
          ...
          { id: toRecordId(decodeURIComponent(fileId)) },
        )
      )[0],
  ).catch(() => undefined);
  if (!file) return new NextResponse("Файл не знайдено", { status: 404 });
  const bytes =
    variant === "preview" && file.previewData ? file.previewData : file.data;
  if (!bytes)
    return new NextResponse("Варіант файлу не знайдено", { status: 404 });
  const body =
  ...
  return new NextResponse(responseBody, {
    headers: {
      ...
      "Content-Disposition":
        variant === "preview" ? "inline" : contentDisposition(file.name),
      "Cache-Control":
        variant === "preview" ? "private, max-age=3600" : "private, no-store",
    },
  });
}
