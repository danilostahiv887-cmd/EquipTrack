    ...
    <section className="module-page">
      <header className="module-heading">
        <div>
          ...
          <h1>Документи</h1>
          <p>
            Фото, сертифікати й службові файли з прив’язкою до карток обліку.
          </p>
        </div>
      </header>
      <form className="filter-line">
        <input
          ...
          placeholder="Назва файлу, тип або прив’язаний об’єкт"
        />
        <select
          name="type"
          defaultValue={search.type ?? ""}
          aria-label="Фільтр за прив’язаною сутністю"
        >
        ...
        <select
          ...
          aria-label="Фільтр за видом файлу"
        >
          <option value="">Усі файли</option>
          {kindOptions.map((kind) => (
            <option key={kind} value={kind}>
              {kind === "photo" ? "Фото" : "Документ"}
            </option>
          ))}
        </select>
          ...
          return (
            <article key={id}>
              <div className="workflow-row-main">
                ...
                <small>
                  {String(item.mimeType ?? "Файл")} ·{" "}
                  {Number(item.size ?? 0).toLocaleString("uk-UA")} байт
                </small>
              </div>
              <span className="workflow-row-date">
                {String(item.targetTypeLabel ?? "Запис")}
              </span>
              {item.targetHref ? (
                <Link href={item.targetHref}>
                  {String(item.targetLabel ?? "Відкрити картку")}
                </Link>
              ) : (
                <span>
                  {String(item.targetLabel ?? "Прив’язку не вказано")}
                </span>
              )}
              <Link
                href={`/api/files/${encodeURIComponent(id)}/full`}
                target="_blank"
              >
                Відкрити файл
              </Link>
      ...
      <Pagination
        ...
        query={{ q: search.q, type: search.type, kind: search.kind }}
      />
    </section>
  );
}
