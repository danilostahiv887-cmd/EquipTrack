...
const transitions: Record<
  string,
  Array<{ status: string; label: string; confirm: string; tone?: "danger" }>
> = {
  reported: [
    {
      status: "under_review",
      label: "Взяти в роботу",
      confirm: "Взяти на перевірку",
    },
    {
      status: "cancelled",
      label: "Скасувати",
      confirm: "Скасувати повідомлення",
      tone: "danger",
    },
  ],
  under_review: [
    {
      ...
      label: "Передати в ремонт",
      confirm: "Передати обладнання в ремонт",
    },
    {
      status: "repaired",
      label: "Повернути в роботу",
      confirm: "Позначити як відремонтоване",
    },
    {
      status: "not_repairable",
      label: "Непридатне",
      confirm: "Позначити як непридатне",
      tone: "danger",
    },
    {
      status: "cancelled",
      label: "Скасувати",
      confirm: "Скасувати ремонт",
      tone: "danger",
    },
  ],
  sent_to_repair: [
    {
      status: "repaired",
      label: "Повернути з ремонту",
      confirm: "Повернути обладнання з ремонту",
    },
    {
      status: "not_repairable",
      label: "Непридатне",
...
export default async function RepairsPage({
  ...
  const [user, rows] = await Promise.all([
    getCurrentUser(),
    getWorkflowPage("repairs", page, search),
  ]);
    ...
    <section className="module-page">
      <header className="module-heading">
        <div>
          ...
          <h1>Ремонти</h1>
          <p>
            Повідомлення про несправності, діагностику й повернення обладнання
            до роботи.
          </p>
        </div>
        <Dialog
          label="Повідомити про несправність"
          title="Нове повідомлення про ремонт"
        >
          <RepairFormLoader />
        </Dialog>
      </header>
      <form className="filter-line">
        <input
          ...
          placeholder="Обладнання, приміщення, опис, серйозність або стан"
        />
        <select
          name="status"
          defaultValue={search.status ?? ""}
          aria-label="Фільтр за станом ремонту"
        >
        ...
        <select
          name="severity"
          defaultValue={search.severity ?? ""}
          aria-label="Фільтр за серйозністю"
        >
      ...
      <WorkflowList
        ...
        actions={(row) =>
          canManage ? (
            <>
              {(transitions[String(row.status ?? "")] ?? []).map(
                (transition) => (
