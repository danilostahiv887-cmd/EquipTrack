...
import {
  AuditFormLoader,
  AuditScanFormLoader,
} from "@/components/workflows/workflow-form-loader";
    ...
    <section className="module-page">
      <header className="module-heading">
        <div>
          ...
          <h1>Аудити</h1>
          <p>
            Перерахунок обладнання в приміщеннях, перевірка фактичного місця й
            стану.
          </p>
        </div>
        <Dialog label="Створити аудит" title="Нова інвентаризація">
          <AuditFormLoader />
        </Dialog>
      </header>
      <form className="filter-line">
        <input
          ...
          placeholder="Назва аудиту, приміщення, обладнання, стан або результат"
        />
        <select
          name="status"
          defaultValue={search.status ?? ""}
          aria-label="Фільтр за станом аудиту"
        >
            ...
            <>
              ...
              {status === "planned" && (
                <form action={startAuditAction}>
                  ...
                  <ConfirmSubmit
                    label="Почати"
                    title="Почати аудит?"
                    description="Стан зміниться на «У роботі», після чого можна буде вносити знайдені серійні номери."
                    confirmLabel="Почати аудит"
                  />
                </form>
              )}
              {status === "in_progress" && (
                <>
                  ...
                  >
                    <AuditScanFormLoader
                      auditId={row.id}
                      auditRoomId={recordId(row.roomId ?? "")}
                    />
                  </Dialog>
                  <form action={finishAuditAction}>
                    ...
                    <ConfirmSubmit
                      label="Завершити"
                      title="Завершити аудит?"
                      description="Неперевірені очікувані екземпляри будуть позначені як відсутні."
                      confirmLabel="Завершити аудит"
                    />
                  </form>
                </>
              )}
              {["planned", "in_progress"].includes(status) && (
                <form action={cancelAuditAction}>
                  ...
                  <ConfirmSubmit
                    label="Скасувати"
                    title="Скасувати аудит?"
                    description="Аудит залишиться в журналі зі станом «Скасовано»."
                    confirmLabel="Скасувати аудит"
                    tone="danger"
                  />
                </form>
              )}
              <form action={deleteAuditAction}>
                ...
                <ConfirmSubmit
                  label="Видалити"
                  title="Видалити аудит?"
                  description="Аудит і всі рядки перевірки будуть видалені без відновлення."
